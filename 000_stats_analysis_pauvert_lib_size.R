...
##########################################################################################################################################################################
# Rscript 000_stats_analysis_pauvert_lib_size.R
# source("000_stats_analysis_pauvert_lib_size.R")
	...
	for (a_marker in unique(full_table$marker)) {
		...
		if (lbl != "large_results_Pauvert") {
			...
			ab_plot[[lbl]] <- ggplot(merged_letters_table, aes(x=treatment, y=abundance, fill=treatment)) +
						...
						scale_fill_manual(values=treatment_palette) +
						guides(fill=guide_legend(title="Treatment", override.aes=list(shape=21, size=10), nrow=1)) +
						geom_text(aes(x=treatment, y=.data$max_y, label=Letter), inherit.aes=F, size=10) +
						ggtitle(NULL) +
						ggplot_theme(leg_pos="bottom", fnt_sz=15) +
						ylab(NULL) +
						xlab(NULL) +
						theme(axis.text.x=element_blank())
		}	
...
# export plot
export_final_svg(paste0(supplementary_figs, "fig_s2"), ab_plot_merge_pauvert, base=F, width=168*3, height=168*2)
