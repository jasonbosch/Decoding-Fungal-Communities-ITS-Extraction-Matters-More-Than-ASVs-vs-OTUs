...
while read a_genome; do
	...
	# find primers
	primersearch -seqall tmp_output.txt -infile ../new_primers.txt -mismatchpercent 20 -outfile found_primers_tmp
	...
	paste -d"," <(grep forward found_primers.txt) <(grep length found_primers.txt) > trimming_positions_alt.txt
	...
	if [ -s trimming_positions.txt ]
		then
			...
			Rscript ../005_substitute_new_primers.R
