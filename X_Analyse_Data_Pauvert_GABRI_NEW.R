# source("X_Analyse_Data_Pauvert_GABRI_NEW.R")
...
final_res <- paste0(root_path, "final_analysis_results_Pauvert/")
...
#readRDS(paste0(root_path, "final_analysis_results_Pauvert/all_alpha_divs.Rds"))
#readRDS(paste0(root_path, "final_analysis_results_Pauvert/all_phylos.Rds"))
#full_taxonomy <- readRDS(paste0(root_path, "final_analysis_results_Pauvert/full_taxonomy_ready.Rds"))
#physeq_combined <- readRDS(paste0(root_path, "final_analysis_results_Pauvert/physeq_combined.Rds"))
#physeq_FWD_ASV <- readRDS(paste0(root_path, "final_analysis_results_Pauvert/physeq_FWD_ASV.Rds"))
#readRDS(paste0(root_path, "final_analysis_results_Pauvert/physeq_FWD_ASV_Rolling.Rds"))
#readRDS(paste0(root_path, "final_analysis_results_Pauvert/physeq_FWD_OTU.Rds"))
#readRDS(paste0(root_path, "final_analysis_results_Pauvert/physeq_ITS_ASV.Rds"))
#readRDS(paste0(root_path, "final_analysis_results_Pauvert/physeq_ITS_ASV_Rolling.Rds"))
#readRDS(paste0(root_path, "final_analysis_results_Pauvert/physeq_ITS_OTU.Rds"))
#physeq_true <- readRDS(paste0(root_path, "final_analysis_results_Pauvert/physeq_true.Rds"))
#Similarity_scores_2 <- readRDS(paste0(root_path, "final_analysis_results_Pauvert/similarity_scores.Rds"))
#true_community <- readRDS(paste0(root_path, "final_analysis_results_Pauvert/true_community_final_clean.Rds"))
#readRDS(paste0(root_path, "final_analysis_results_Pauvert/true_tax_table.Rds"))

#setwd("~/PostDoc/02_Projects/06_ASV_OTU_Comparison/04_Analysis_Results/Pauvert/")
...
#Shared
metadata_raw <- read.table(paste0(root_path, "Metadata_Pauvert.tsv"), 
...
#Import
table_FWD_ASV_Rolling_raw <- read.table(paste0(root_path, "FWD_ASV_Rolling_Pauvert/export/OTU.tsv"), 
                                header = TRUE, row.names = 1, sep = "\t", comment.char = "#")
taxonomy_FWD_ASV_Rolling <- import_qiime2_taxonomy(read.table(paste0(root_path, "FWD_ASV_Rolling_Pauvert/export/taxonomy.tsv"), header = TRUE, row.names = 1, sep = "\t"))

#Filtering
#Remove any ASV with fewer 0.1% of the mean number of reads per sample per library size
table_FWD_ASV_Rolling_final <- otu_table(table_FWD_ASV_Rolling_raw[(rowSums(table_FWD_ASV_Rolling_raw)>round(mean(colSums(table_FWD_ASV_Rolling_raw))*0.001)),],taxa_are_rows = TRUE)
#46 taxa removed
...
#Import
table_FWD_ASV_raw <- read.table(paste0(root_path, "FWD_ASV_Pauvert/export/OTU.tsv"), 
                                 header = TRUE, row.names = 1, sep = "\t", comment.char = "#")
taxonomy_FWD_ASV <- import_qiime2_taxonomy(read.table(paste0(root_path, "FWD_ASV_Pauvert/export/taxonomy.tsv"), header = TRUE, row.names = 1, sep = "\t"))

#Filtering
#Remove any ASV with fewer 0.1% of the mean number of reads per sample per library size
table_FWD_ASV_final <- otu_table(table_FWD_ASV_raw[(rowSums(table_FWD_ASV_raw)>round(mean(colSums(table_FWD_ASV_raw))*0.001)),],taxa_are_rows = TRUE)
#43 taxa removed
...
#Import
table_FWD_OTU_raw <- read.table(paste0(root_path, "FWD_OTU_Pauvert/export/OTU-dn-97.tsv"), 
                                header = TRUE, row.names = 1, sep = "\t", comment.char = "#")
taxonomy_FWD_OTU <- import_qiime2_taxonomy(read.table(paste0(root_path, "FWD_OTU_Pauvert/export/taxonomy.tsv"), header = TRUE, row.names = 1, sep = "\t"))

#Filtering
table_FWD_OTU_final <- otu_table(table_FWD_OTU_raw[(rowSums(table_FWD_OTU_raw)>round(mean(colSums(table_FWD_OTU_raw))*0.001)),],taxa_are_rows = TRUE)
#988 taxa removed
...
#Import
table_ITS_ASV_Rolling_raw <- read.table(paste0(root_path, "ITS_ASV_Rolling_Pauvert/export/OTU.tsv"), 
                                header = TRUE, row.names = 1, sep = "\t", comment.char = "#")
taxonomy_ITS_ASV_Rolling <- import_qiime2_taxonomy(read.table(paste0(root_path, "ITS_ASV_Rolling_Pauvert/export/taxonomy.tsv"), header = TRUE, row.names = 1, sep = "\t"))

#Filtering
table_ITS_ASV_Rolling_final <- otu_table(table_ITS_ASV_Rolling_raw[(rowSums(table_ITS_ASV_Rolling_raw)>round(mean(colSums(table_ITS_ASV_Rolling_raw))*0.001)),],taxa_are_rows = TRUE)
#57 taxa removed
...
#1 Eukaryota_kgd_Incertae_sedis, 1 Unassigned
...
#Import
table_ITS_ASV_raw <- read.table(paste0(root_path, "ITS_ASV_Pauvert/export/OTU.tsv"), 
                                header = TRUE, row.names = 1, sep = "\t", comment.char = "#")
taxonomy_ITS_ASV <- import_qiime2_taxonomy(read.table(paste0(root_path, "ITS_ASV_Pauvert/export/taxonomy.tsv"), header = TRUE, row.names = 1, sep = "\t"))

#Filtering
table_ITS_ASV_final <- otu_table(table_ITS_ASV_raw[(rowSums(table_ITS_ASV_raw)>round(mean(colSums(table_ITS_ASV_raw))*0.001)),],taxa_are_rows = TRUE)
#57 taxa removed
...
#Import
table_ITS_OTU_raw <- read.table(paste0(root_path, "ITS_OTU_Pauvert/export/OTU-dn-97.tsv"), 
                                header = TRUE, row.names = 1, sep = "\t", comment.char = "#")
taxonomy_ITS_OTU <- import_qiime2_taxonomy(read.table(paste0(root_path, "ITS_OTU_Pauvert/export/taxonomy.tsv"), header = TRUE, row.names = 1, sep = "\t"))

#Filtering
table_ITS_OTU_final <- otu_table(table_ITS_OTU_raw[(rowSums(table_ITS_OTU_raw)>round(mean(colSums(table_ITS_OTU_raw))*0.001)),],taxa_are_rows = TRUE)
#420 taxa removed
...
tcommunity <- read.table("/mnt/cinqueg/gabriele/work/microbiology/other_analyses/jason/data/1-s2.0-S1754504818302800-mmc2.csv",header = T, sep = ";")

true_community <- tcommunity
#Community is in equal proportions
true_community$Count <- 100/nrow(true_community)
#Expected number of reads for large library
true_community$reads <- round(true_community$Count/100*(0.5*50000))
#Three identical libraries
true_community <- true_community[,c(1,13,13,13)]
colnames(true_community) <- c("V2","ITSEnz.DikA","ITSEnz.DikB","ITSEnz.DikC")
#Fix an entry with a double underscore
true_community$V2 <- gsub("__","_",true_community$V2)

#Import full taxonomy
# cut -f2 sh_taxonomy_qiime_ver9_dynamic_all_25.07.2023_dev.txt | sed 's/[a-z]__//g' | tail -n +2 | cut -d";" -f1-7 | sort -u > taxonomy_qiime.csv
...
#Filtering
true_community_final <- otu_table(true_community[(rowSums(true_community)>round(mean(colSums(true_community))*0.001)),],taxa_are_rows = TRUE)
#0 taxa removed
...
# load data
#combined_alphadiv_df <- readRDS(paste0(final_res, "combined_alphadiv_df.Rds"))[["Large"]]
...
physeq_true_proportion@sam_data$Grouping <- factor(physeq_true_proportion@sam_data$Grouping, levels = sort(unique(physeq_true_proportion@sam_data$Grouping))[c(1,2,3)], ordered = T)
...
physeq_genus <- merge_taxa(physeq_genus,taxa_names(filter_taxa(physeq_genus, function(x) mean(x) < 0.01, TRUE)))
...
saveRDS(Similarity_scores_2, paste0(root_path, "final_analysis_results_Pauvert/similarity_scores.Rds"))
...
Sensitivity_legend_2<- get_plot_component(plot_sensitivity_2, 'guide-box-bottom', return_all = TRUE)
Sensitivity_plots_2 <-plot_grid(plot_grid(plot_sensitivity_2 + theme(legend.position = "none"),
                                        plot_precision_2 + theme(legend.position = "none"),
                                        plot_aitchison_2 + theme(legend.position = "none"),
                                        plot_hellinger_2 + theme(legend.position = "none"),
                                        plot_bray_2 + theme(legend.position = "none"),
