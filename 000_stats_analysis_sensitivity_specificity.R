...
##########################################################################################################################################################################
# Rscript 000_stats_analysis_sensitivity_specificity.R
# source("000_stats_analysis_sensitivity_specificity.R")
...
#	a_dir <- "final_analysis_results_gITS7_ITS4" ; # a_dir <- "final_analysis_results_Pauvert"
	...
	if (a_dir == "final_analysis_results_Pauvert") {
		...
		# plot
		plot_sensitivity <- ggplot(similarity_scores[similarity_scores$Level != "Kingdom", ], aes(x=Level, y=Sensitivity, group=Treatment, colour=Treatment)) +
					stat_summary(position=position_dodge2(width=0.5), fun="mean", geom="line") +
					geom_point(aes(fill=Treatment), colour="black", size=10, shape=21, position=position_dodge(width=0.5)) +
					scale_colour_manual(values=treatment_palette, labels=gsub("_", " ", unique(similarity_scores$Treatment))) +
					scale_fill_manual(values=treatment_palette, labels=gsub("_", " ", unique(similarity_scores$Treatment))) +
					ggtitle("") +
		...
		# export treatmentsimilarity_scores
		export_final_svg(paste0(supplementary_figs, "fig_S36"), plot_sensitivity, base=F, width=168*4, height=168*2)

		# plot
		plot_specificity <-  ggplot(similarity_scores[similarity_scores$Level != "Kingdom", ], aes(x=Level, y=Specificity, group=Treatment, colour=Treatment)) +
					stat_summary(position=position_dodge2(width=0.5), fun="mean", geom="line") +
					geom_point(aes(fill=Treatment), colour="black", size=10, shape=21, position=position_dodge(width=0.5)) +
					scale_colour_manual(values=treatment_palette, labels=gsub("_", " ", unique(similarity_scores$Treatment))) +
					scale_fill_manual(values=treatment_palette, labels=gsub("_", " ", unique(similarity_scores$Treatment))) +
		...
		# export treatment
		export_final_svg(paste0(supplementary_figs, "fig_S37"), plot_specificity, base=F, width=168*4, height=168*2)

		# make summary table and export it
		similarity_scores %>%
			group_by(Level, Treatment, Diversity, Library_Size, Method) %>%
			summarise(specificity_mean = mean(Specificity), sensitivity_mean = mean(Sensitivity)) %>%
			data.frame() %>%
			write.table(paste0(save_means, a_dir, "_sensib_specif_means.csv"), quote=F, sep="\t", row.names=F)
		...
		# export plot
		count <- ifelse(a_dir=="final_analysis_results_ITS1f_ITS2", 16, 17)

		# export treatment
		export_final_svg(paste0(supplementary_figs, "fig_S", count), plot_sensitivity, base=F, width=168*6, height=168*6)
		# export treatment
		export_final_svg(paste0(supplementary_figs, "fig_S", count+2), plot_specificity, base=F, width=168*6, height=168*6)

		# subset the data structure for plotting purposes
		similarity_scores_subset_by_lib_div <- similarity_scores[similarity_scores$Library_Size=="Large" & similarity_scores$Diversity==500, ]
		# merge similarity scores
		subtable <- similarity_scores_subset_by_lib_div[, c("Diversity", "Library_Size", "Replicate", "Processing", "Method", "Treatment", "Level", "Sensitivity", "Specificity")]
		similarity_scores_subset_by_lib_div_long <- cbind.data.frame(rbind.data.frame(subtable, subtable, subtable), similarity_score=c(similarity_scores_subset_by_lib_div$Aitchison, similarity_scores_subset_by_lib_div$Hellinger, similarity_scores_subset_by_lib_div$BrayCurtis), score_name=c(rep("Aitchison", nrow(similarity_scores_subset_by_lib_div)), rep("Hellinger", nrow(similarity_scores_subset_by_lib_div)), rep("Bray-Curtis", nrow(similarity_scores_subset_by_lib_div))))

		# plot
		no_kingdom <- similarity_scores_subset_by_lib_div_long[similarity_scores_subset_by_lib_div_long$Level!="Kingdom", ]
		plot_sim_scores <- ggplot(no_kingdom, aes(x=Level, y=similarity_score, group=Treatment, colour=Treatment)) +
				stat_summary(position=position_dodge2(width=0.5), fun="mean", geom="line", lwd=1) +
				stat_summary(position=position_dodge2(width=0.5), fun.data="mean_sdl", fun.args=list(mult=1), geom="errorbar", width=0.5, lwd=1) +
				scale_colour_manual(values=treatment_palette) +
				guides(colour=guide_legend(nrow=3)) +
				ggtitle(NULL) +
				ggplot_theme(leg_pos="bottom", ang_le=0, fnt_sz=15, top=0.2, right=0.2, bottom=0.2, left=0.2) +
				ylab(NULL) +
				xlab(NULL) +
		...
		if (a_dir == "final_analysis_results_gITS7_ITS4") {
			export_final_svg(paste0(root_path, "tables_final/fig_3"), plot_sim_scores, base=F, width=80*2, height=237)
		}
		# make summary table and export it
		similarity_scores %>%
			group_by(Level, Treatment, Diversity, Library_Size, Method) %>%
			summarise(specificity_mean = mean(Specificity), sensitivity_mean = mean(Sensitivity)) %>%
			data.frame() %>%
			write.table(paste0(save_means, a_dir, "_sensib_specif_stats.csv"), quote=F, sep="\t", row.names=F)
